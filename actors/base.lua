...
local SpriteComponent = Component:extend{}


local HealthComponent = Component:extend{}

function HealthComponent:init(max_health)
...
function HealthComponent:damage(actor, amount, type, source)
...
function HealthComponent:on_die(actor, killer)
    actor:destroy()
end


local InteractComponent = Component:extend{}

function InteractComponent:on_interact(actor, activator)
...
local GravityComponent = Component:extend{
    terminal_speed = 1536,
}

function GravityComponent:init()
...
function GravityComponent:get_base_gravity()
...
-- FIXME gravity affects get_friction, which, arguably, IS a property of gravity,

function GravityComponent:act(actor, dt)
...
function GravityComponent:after_collisions(actor, movement, collisions)
    self:check_for_ground(actor, collisions)
end

function GravityComponent:check_for_ground(actor, hits)
...
local SentientGravityComponent = GravityComponent:extend{}

function SentientGravityComponent:init(max_slope, ...)
    SentientGravityComponent.__super.init(self, ...)
    self.max_slope = max_slope
end

function SentientGravityComponent:check_for_ground(actor, ...)
    SentientGravityComponent.__super.check_for_ground(self, actor, ...)
...
function SentientGravityComponent:act(actor, dt)
    SentientGravityComponent.__super.act(self, actor, dt)
...
function SentientGravityComponent:after_collisions(actor, movement, collisions)
    local was_on_ground = self.grounded

    SentientGravityComponent.__super.after_collisions(self, actor, movement, collisions)
...
function MobileActor:init(...)
    ...
    self.gravity_component = GravityComponent()
end
...
function MobileActor:get_friction(normalized_direction)
    ...
    elseif self.gravity_component.ground_normal then
        local gravity1 = self.gravity_component:get_base_gravity():normalized()
        -- Get the strength of the normal force by dotting the ground normal
        -- with gravity
        local normal_strength = gravity1 * self.gravity_component.ground_normal * self:_get_carried_mass()
        friction = self.gravity_component.ground_normal:perpendicular() * (self.friction_decel * self.gravity_component.ground_friction * normal_strength)
...
function MobileActor:_collision_callback(collision, pushers, already_hit)
    ...
    if actor and
        ...
        self.gravity_component.grounded and
...
function MobileActor:update(dt)
    ...
    -- FIXME should be in GravityComponent
...
-- ========================================================================== --
-- Components...?
local WalkComponent = Object:extend{
    decision = Vector.zero,
}

-- TODO air acceleration doesn't make sense for 2D, maybe?
function WalkComponent:init(ground_acceleration, air_acceleration, deceleration_multiplier, max_speed)
...
function WalkComponent:decide(dx, dy)
    self.decision = Vector(dx, dy)
    self.decision:normalizeInplace()
end

function WalkComponent:act(actor, dt)
...
local JumpComponent = Object:extend{
    consecutive_jump_count = 0,
    decision = 0,
}

-- FIXME needs to accept max jumps
function JumpComponent:init(jump_speed, abort_speed, sound)
    self.jump_speed = jump_speed
    self.abort_speed = abort_speed
    self.sound = sound
end

function JumpComponent:decide(whether)
...
function JumpComponent:act(actor)
    if actor.gravity_component.grounded then
...
local ClimbComponent = Component:extend{}

function ClimbComponent:init(speed)
    self.speed = speed
...
-- nil to let go.
function ClimbComponent:decide(direction)
...
function ClimbComponent:after_collisions(actor, movement, collisions)
...
function ClimbComponent:act(actor, dt)
...
local InteractorComponent = Component:extend{}

function InteractorComponent:decide()
    self.decision = true
end

function InteractorComponent:act(actor, dt)
...
function SentientActor:init(...)
    ...
    self.walk_component = WalkComponent(self.xaccel, self.xaccel * self.aircontrol, self.deceleration, self.max_speed)
    self.jump_component = JumpComponent(self.jumpvel, self.jumpvel * self.jumpcap, game.resource_manager:get(self.jump_sound))
    self.climb_component = ClimbComponent(self.climb_speed)
    self.interactor_component = InteractorComponent()
    self.gravity_component = SentientGravityComponent(self.max_slope)
end
...
function SentientActor:determine_pose()
    ...
    elseif self.gravity_component.grounded then
